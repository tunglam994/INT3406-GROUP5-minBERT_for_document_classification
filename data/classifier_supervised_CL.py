...
class Loss():
    ...
    def train_loss_fct(self, criterion, a, p, n, neg_weight=0):
        # a: tensor shape (batch_size, embedding_dim)
...
class BertBooksClassifier(torch.nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        # output: {'last_hidden_state': tensor of shape (batch_size, sequence_length, hidden_size), 'pooler_output': tensor of shape (batch_size, hidden_size)}
        output = self.bert(input_ids, attention_mask)
        # pooled: tensor of shape (batch_size, hidden_size)
        pooled = output['pooler_output']
        # mask = torch.ones_like(pooled)
...
class BertDataset(Dataset):
    ...
    def pad_data(self, data):
        #data: triplet of (sentence, label, token for the labels)
        ...
        # token_type_ids_anchor = torch.LongTensor(encoding_anchor['token_type_ids'])
        ...
        # token_type_ids_pos = torch.LongTensor(encoding_pos['token_type_ids'])
        ...
        # token_type_ids_neg = torch.LongTensor(encoding_neg['token_type_ids'])
        ...
        #sents: tensor of shape (batch_size)
    ...
    def collate_fn(self, all_data):
        ...
        for i in range(num_batches):
            ...
            # b_ids, b_type_ids, b_mask, b_sents
            batches.append({
...
def create_data(filename, flag='train'):
    ...
    #         # # Extract anchor, positive, and negative sentences
    #         # anchor, pos, neg = parts[1], parts[2], parts[3]
...
#             # Extract anchor, positive, and negative sentences
#             anchor, positive, negative = parts[1], parts[2], parts[3]

#             # Tokenize sentences
#             tokens_anchor = tokenizer.tokenize("[CLS] " + anchor.lower().strip() + " [SEP]")
#             tokens_positive = tokenizer.tokenize("[CLS] " + positive.lower().strip() + " [SEP]")
#             tokens_negative = tokenizer.tokenize("[CLS] " + negative.lower().strip() + " [SEP]")

#             # Append to lists
#             anchors.append(anchor)
#             tokens_anchors.append(tokens_anchor)
#             pos.append(positive)
#             tokens_pos.append(tokens_positive)
#             negav.append(negative)
#             tokens_negav.append(tokens_negative)

#     print(f"Loaded {len(anchors)} data entries from {filename}")
    
#     # if flag == 'train':
#     #     return anchors, tokens_anchors, pos, tokens_pos, negav, tokens_negav
#     # else:
#     return anchors, tokens_anchors, pos, tokens_pos, negav, tokens_negav
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc=f'train-{epoch}', disable=TQDM_DISABLE)):
            ...
            # b_ids: [batch_size, seq_len], b_mask: [batch_size, seq_len], b_labels: [batch_size]
